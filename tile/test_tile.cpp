...
for(int i = 0; i < ig.size(); i ++){  
   ...
   registration rg;
   vector< pair<float, float> > peak = rg.getIRPosition(im, 0.5, ala, bim, true); 
...
//if random, then make sample for rotation and scale after FFT, use the dot pair of ATCG image and corresponding the dot pair of reference map image to compute linear distortion
...
for(int i = 0; i < ig.size(); i ++){ 
   ...
   registration rg;
   vector< vector<long long> > bim;
   peak = rg.getIRPosition(ref, 0.1, ala, bim, false);/////////////////////////////////   
